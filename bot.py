...
DEFAULT_FEEDS = {
    ...
    "feeds": [
        ...
        {"name": "یورونیوز فارسی", "url": "https://fa.euronews.com/rss?level=theme&name=news", "lang": "fa", "enabled": True, "og_image": True},
...
def ensure_feeds() -> None:
    ...
    og_migrated = False
    for f in feeds:
        if "euronews" in str(f.get("url", "")) and not f.get("og_image"):
            f["og_image"] = True
            og_migrated = True
    if added or og_migrated:
        save_json(FEEDS_FILE, cfg)
    if added:
        print("[info] منابع تلگرامی جدید به feeds اضافه شد")
    if og_migrated:
        print("[info] گزینهٔ og_image برای یورونیوز فعال شد (عکسِ هر خبر از صفحهٔ آن خوانده می‌شود)")
# -------------------------------------------------------------- ابزارها ---
    ...
    for b in raw.split("tgme_widget_message_wrap")[1:]:
        ...
        if time_m:
            try:
                ...
                dt = None
        # عکس پیوست خودِ همان پست (پیش‌نمایش t.me) تا خبر با عکسِ مربوط به خودش برود
        img = ""
...
OG_CACHE_FILE = STATE_DIR / "og_cache.json"
OG_FRESH_DAYS = 7

def og_image(url: str, cache: dict) -> str:
    """عکس شاخص صفحهٔ خبر از متای og:image/twitter:image با کش ۷ روزه."""
    url = clean_link(url)
    if not url.startswith(("http://", "https://")):
        return ""
    now = int(time.time())
    hit = cache.get(url)
    if isinstance(hit, dict) and now - int(hit.get("t", 0)) < OG_FRESH_DAYS * 86400:
        return str(hit.get("v") or "")
    val = ""
    try:
        ...
        if r.status_code == 200 and "html" in (r.headers.get("Content-Type") or "").lower():
            for m in re.finditer(r"<meta\b[^>]*>", r.text, re.I):
                tag = m.group(0)
                if re.search(r'\b(?:property|name)\s*=\s*["\'](?:og:image|twitter:image)["\']',
                             tag, re.I):
                    c = re.search(r'\bcontent\s*=\s*["\']([^"\']+)["\']', tag, re.I)
                    if c:
                        val = html.unescape(c.group(1)).strip()
                        break
            if val.startswith("//"):
                val = "https:" + val
...
def collect_items(cfg: dict) -> list[dict]:
    ...
    need_og = any(f.get("og_image") for f in cfg.get("feeds", []) if f.get("enabled", True))
    og_cache = load_json(OG_CACHE_FILE, {}) if need_og else {}
    for feed in cfg.get("feeds", []):
        if not feed.get("enabled", True):
            continue
        name = feed["name"]
        is_tg = bool(feed.get("channel"))
        is_region_feed = bool(feed.get("region", False)) or is_tg
        entries = fetch_tg_channel(feed) if is_tg else fetch_feed(feed)
        ...
        for entry in entries:
            ...
            # عکس شاخص خبر: در فیدهای RSS از media:content / media:thumbnail / enclosure
            image = ""
            if not is_tg:
                for cand in (entry.get("media_content") or []):
                    u = cand.get("url") or ""
                    if u and ("image" in (cand.get("medium") or "").lower()
                              or "image" in (cand.get("type") or "")
                              or re.search(r"\.(jpe?g|png|gif|webp)(?:[?#]|$)", u, re.I)):
                        image = u
                        break
                if not image:
                    for cand in (entry.get("media_thumbnail") or []):
                        u = cand.get("url") or ""
                        if u:
                            image = u
                            break
                if not image:
                    for enc in (entry.get("enclosures") or []):
                        if (enc.get("type") or "").startswith("image/"):
                            image = enc.get("href") or enc.get("url") or ""
                            break
                if not image:
                    # آخرین تلاش: نخستین <img> درون HTML خودِ خبر
                    m_html = re.search(r'<img[^>]+src=["\']([^"\']+)["\']',
                                       summary_raw or "", re.I)
                    if m_html:
                        image = m_html.group(1)
                image = clean_link(image)
                if image.startswith("//"):
                    image = "https:" + image
            ...
            if region_filter and not is_region_feed and not REGION_RE.search(title + " " + summary):
                dropped_region += 1
                continue
            # فید بی‌رسانه با گزینهٔ og_image: عکسِ شاخصِ خودِ همان صفحهٔ خبر را می‌خواند
            if not image and not is_tg and feed.get("og_image"):
                link_tmp = clean_link(entry.get("link") or "")
                if link_tmp.startswith(("http://", "https://")):
                    image = og_image(link_tmp, og_cache)
            items.append({
                ...
                "source": name,
                "published": published,
                "image": entry.get("image") or image,
            })
    if dropped_region:
    ...
    unique.sort(key=lambda x: x["published"] or datetime.min.replace(tzinfo=timezone.utc),
                reverse=True)
    if need_og and og_cache:
        now = int(time.time())
        for k in [k for k, v in og_cache.items()
                  if isinstance(v, dict) and now - int(v.get("t", 0)) > OG_FRESH_DAYS * 86400]:
            og_cache.pop(k, None)
        save_json(OG_CACHE_FILE, og_cache)
    return unique
...
def _fixup_entities(text: str) -> str:
    """هر & که سرِ یک نهاد (entity) کامل نیست، به &amp; تبدیل شود تا

    کپشنِ بریده‌شدهٔ عکس خطای «پارس HTML» ندهد."""
    return re.sub(r"&(?!(?:amp|lt|gt|quot|#\d+|#x[0-9a-fA-F]+);)", "&amp;", text)

def _cut_body(body: str, budget: int) -> str:
    """بریدن متن از جای امن: سر کلمه، بدون نهادِ HTMLِ ناقص در انتها."""
    chunk = body[:budget]
    sp = chunk.rfind(" ")
    if sp > budget * 0.5:  # فقط اگر نقطهٔ برشِ طبیعی نزدیکِ انتها باشد
        chunk = chunk[:sp]
    chunk = re.sub(r"&(?!(?:amp|lt|gt|quot|#\d+|#x[0-9a-fA-F]+);)\S*$", "", chunk)
    return chunk.strip(" .،:")

def fit_caption(text: str, limit: int = 1000) -> str:
    """کپشن عکس حداکثر ۱۰۲۴ کاراکتر است؛ اگر متن خبر بلندتر بود،

    پانوشت (زمان/منبع/لینک) دست‌نخورده می‌ماند و از وسطِ متن کم می‌شود."""
    if len(text) <= limit:
        return text
    marker = text.find("\n🕒")
    if marker == -1:
        return _cut_body(text, limit) + "…"
    body, foot = text[:marker], text[marker:]
    budget = limit - len(foot) - 1
    if budget < 80:
        return _fixup_entities(foot[:limit])
    return _cut_body(body, budget) + "…\n" + foot.lstrip("\n")
# ---------------------------------------------------- مقصدها و تنظیمات ---
...
PHOTO_MAX_BYTES = 9_500_000  # تلگرام حداکثر ۱۰ مگابایت برای عکس می‌پذیرد

def download_image(url: str, timeout: int = 20, limit: int = PHOTO_MAX_BYTES) -> bytes:
    """دانلود عکسِ همان خبر با محدودیت حجم و بررسی تصویری بودن پاسخ."""
    if not str(url).startswith(("http://", "https://")):
        raise ValueError("نشانی نامعتبر")
    ...
    if len(data) > limit:
        raise ValueError(f"حجم زیاد ({len(data)} بایت)")
    head = data[:12].lower()
    ok = head.startswith((b"\xff\xd8", b"\x89PNG", b"GIF8", b"RIFF", b"WEBP"))
    if not ok:
        raise ValueError("پاسخ تصویر نیست")
    return data

def _send_item(tg: Tg, dest: dict, text: str, img_url: str,
               img_cache: dict[str, bytes]) -> dict:
    """ارسال هر خبر به یک مقصد: اگر عکسِ همان خبر در دسترس بود، «عکس + متن»؛

    وگرنه متن تنها (رفتار قبلی) تا خبر هرگز از دست نرود."""
    if img_url:
        try:
            if img_url in img_cache:
                data = img_cache[img_url]
            else:
                data = download_image(img_url)
                img_cache[img_url] = data
            r = tg.send_photo_file(dest["id"], data, caption=fit_caption(text))
            if r.get("ok"):
                r["kind"] = "photo"
                return r
            print(f"  ⚠ عکس خبر ارسال نشد ({r.get('description')}) — متن تنها فرستاده می‌شود")
        except Exception as exc:  # noqa: BLE001
            print(f"  ⚠ عکس خبر دریافت نشد ({img_url[:90]}): {exc} — متن تنها فرستاده می‌شود")
    return tg.send(dest["id"], text)
...
def publish_once(tg: Tg, dry_run: bool = False, limit: int | None = None,
    ...
    if first_run:
        ...
        fresh = fresh[:limit]
    stats = {"new_total": len(items), "fresh": len(fresh), "sent": 0, "photos": 0, "errors": 0,
             "dests": len(dests)}
    if not fresh:
    ...
    for item in fresh:
        ...
        if img_url and dry_run:
            print(f"[info] عکس خبر: {img_url[:110]}")
        if dry_run:
            print("=" * 50)
    ...
    photo_note = f" ({stats['photos']} تا با عکس)" if stats["photos"] else ""
    print(f"[info] {stats['sent']} خبر به {len(dests)} مقصد منتشر شد{photo_note}.")
    sync_state()
    return stats
# ------------------------------------------------------------ تصویر روز ---
    ...
    if "callback_query" in up:
        ...
        if data == "m:main":
            ...
            if stats.get("error"):
                ...
                if stats.get("photos"):
                    extra += f"\n🖼 {to_digits(str(stats['photos']))} خبر با عکسِ خودِ خبر فرستاده شد"
